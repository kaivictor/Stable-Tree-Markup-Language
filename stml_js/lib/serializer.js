...
const { nullNode, stringNode, listNode, mapNode, mapFind } = require('./ast');
...
function valuesAreAllScalars(lst) {
  for (const v of lst) {
...
function serializeNode(node, indentLevel = 0) {
...
function toJsonImpl(node, indentLevel = 0) {
  const indent = ' '.repeat(indentLevel * 2);
  const nextIndent = ' '.repeat((indentLevel + 1) * 2);
...
module.exports = { serializeNode, dumps, toJson, escapeString, quoteKey, formatScalar };